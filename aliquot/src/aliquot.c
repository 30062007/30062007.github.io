#include <stdio.h>  // Εισαγωγή βιβλιοθηκης

long long seen[1000];     // Πίνακας για ανίχνευση κύκλων 
int seen_count = 0;       // Μετράει πόσοι αριθμοί έχουν αποθηκευτεί

// Υπολογίζει την ακέραιη τετραγωνική ρίζα του n (χωρίς χρήση math.h)
...
long long aliquot_sum(long long n) {
    if (n <= 1)
        return 0;
    long long sum = 1; // Το 1 είναι πάντα διαιρέτης
    long long sqrt_n = int_sqrt(n);
